...
export const CustomVideoPlayer = ({ url, playing: forcePlaying = true }: CustomVideoPlayerProps) => {
  ...
  const safePlay = () => {
    if (playerRef.current) {
        setPlaying(true);
        // Using setTimeout to let React update the DOM if needed.
        setTimeout(() => {
            const p = playerRef.current?.play && playerRef.current.play();
  ...
  const handleToggleMuted = (e: React.MouseEvent) => {
    ...
    if (muted) {
      ...
      setMuted(true);
      // Wait for state updates then ensure video stays playing
      safePlay();
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-10 pointer-events-none" onClick={handlePlayPause} style={{ pointerEvents: 'auto' }} />
      ...
      <div className="w-full h-full pointer-events-none" style={{ pointerEvents: 'none' }}>
          <ReactPlayer 
            ...
            onTimeUpdate={(e) => {
              ...
              if (duration > 0) {
                 setPlayed(target.currentTime / duration);
              }
              if (target.buffered && target.buffered.length > 0) {
                 setLoaded(target.buffered.end(target.buffered.length - 1) / duration);
              }
            }}
            ...
            onPause={() => {
              // Ignore pause events triggered by buffering or attribute syncing
            }}
      ...
      <AnimatePresence>
        {showControls && (
          <motion.div 
            ...
            className="absolute bottom-0 left-0 right-0 z-30 p-6 bg-gradient-to-t from-black via-black/60 to-transparent flex flex-col gap-4 pointer-events-auto"
          ...
          >
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-6">
                    <button onClick={handlePlayPause} className="text-[#E5D2A5] hover:text-white transition-colors focus:outline-none focus:ring-0">
                        {playing ? <Pause className="w-7 h-7 fill-current" /> : <Play className="w-7 h-7 fill-current" />}
                    </button>
                    
                    <div className="flex items-center gap-3 group/volume">
                        <button onClick={handleToggleMuted} className="text-white hover:text-[#E5D2A5] transition-colors focus:outline-none">
                ...
                <div>
                    <button onClick={handleToggleFullscreen} className="text-white hover:text-[#E5D2A5] transition-colors focus:outline-none">
